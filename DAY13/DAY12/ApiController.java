...
public class ApiController {
	...
	private final JobDetailsRepository jobrepo;
	@GetMapping("/user/get")
	...
	@GetMapping("/user/putsaved/{id}/{email}")
	public void updateUserSaved(@PathVariable int id,@PathVariable String email) {
		 apiService.updateUserSaved(id,email);
	}
	...
	@GetMapping("/user/getapplied/{id}")
	public List<User> getapplieduser(@PathVariable int id) {
		JobDetails job=apiService.getJobDetails(id);
		List<Integer> userids=job.getUser();
		List<User> users=new ArrayList<>();
		for(int i: userids) {
         	User user=apiService.getUser(i);
         	users.add(user);
		}
		return users;
	...
	@GetMapping("/savedjob/putbyemail/{email}/{id}")
	...
	@GetMapping("/applied/put/{email}/{id}")
	public void updateUserapplied(@PathVariable String email,@PathVariable int id) {
		 ...
		 for(JobDetails j:jobs) {
			 if(j.getId()==id) {
				 List<Integer> appliedjobs= j.getUser();
				 if(appliedjobs==null) {
					 appliedjobs= new ArrayList<>();
				 }
				 appliedjobs.add(user.getId());
				 j.setUser(appliedjobs);
				 temp.add(j);  
				 jobrepo.save(j);
			 }
		 }
